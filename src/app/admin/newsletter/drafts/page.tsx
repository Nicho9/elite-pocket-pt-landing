    ...
    async function loadDrafts() {
      ...
      try {
        ...
        if (!response.ok || !payload.success) {
          setErrorMessage(payload.success ? "Could not load newsletter drafts." : payload.error);
          setDrafts([]);
        } else {
        ...
        if (isMounted) {
          setErrorMessage(error instanceof Error ? error.message : "Could not load newsletter drafts.");
          setDrafts([]);
        }
      } finally {
        if (isMounted) {
      ...
      <section className="mx-auto w-full max-w-7xl">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex flex-col gap-3">
            ...
            <p className="max-w-2xl text-base font-medium text-[#4B5563]">
              Saved newsletter campaigns waiting for review or delivery.
            </p>
          </div>
        ...
        <div className="mt-8 overflow-hidden rounded-[1.5rem] border border-[#E5E7EB] bg-white shadow-[0_18px_44px_rgba(15,23,42,0.08)]">
          <div className="border-b border-[#E5E7EB] px-5 py-4">
            <h2 className="text-lg font-bold text-[#0B1220]">Saved drafts</h2>
            <p className="mt-1 text-sm font-medium text-[#6B7280]">
              Review campaign drafts before adding delivery controls.
            </p>
          </div>

          {isLoading ? (
            <p className="px-5 py-8 text-sm font-semibold text-[#4B5563]">
              Loading newsletter drafts...
            </p>
          ) : drafts.length === 0 ? (
            <div className="px-5 py-10">
              <h2 className="text-xl font-bold text-[#0B1220]">No draft campaigns yet</h2>
              <p className="mt-2 max-w-2xl text-sm font-medium leading-6 text-[#4B5563]">
                Saved drafts will appear here once a campaign has been created.
              </p>
            </div>
          ) : (
                  ...
                  <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-start">
                    <div>
                      ...
                      <div className="mt-4 grid gap-3 text-sm font-semibold text-[#4B5563] md:grid-cols-2 xl:grid-cols-5">
                        <div>
                          ...
                          <p className="mt-1 text-[#0B1220]">{titleCase(draft.campaign_type)}</p>
                        </div>
                        <div>
                          <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#94A3B8]">
                            Audience
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-[#0B1220]">{formatDateTime(draft.created_date)}</p>
                        </div>
                        <div>
                          <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#94A3B8]">
                            Updated date
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row xl:flex-col">
                      ...
                      <button
                        type="button"
                        disabled
                        className="h-10 cursor-not-allowed rounded-xl border border-[#E5E7EB] bg-white px-4 text-sm font-bold text-[#9CA3AF]"
                      >
